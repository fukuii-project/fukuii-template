...
#         2  cannot run (not inside a git work tree)
#
...
printf '| `%s` | %s |\n' "$SEMGREP" "$([ -e "$SEMGREP" ] && echo present || echo removed)"
printf '| `%s` | %s |\n' "$TRIVY" "$([ -e "$TRIVY" ] && echo present || echo removed)"
printf '| `.npmrc` | %s |\n' "$([ -e .npmrc ] && echo present || echo absent)"
printf '\n'
if [ "$BRANCHING_BULLET" = yes ]; then
  printf '**VERDICT: ALREADY-ONBOARDED.** The Branching bullet is the sentinel — it is\n'
  printf 'the one edit every answer produces, so its presence means the interview ran.\n'
...
printf 'A tracked file matching HEAD is a REVERSIBLE write: git holds the pre-image.\n'
printf 'Anything else is IRREVERSIBLE and /onboard must refuse to delete it.\n\n'
printf '| File | Verdict | Why |\n|---|---|---|\n'

delete_safety() {
  local path="$1"
  if [ ! -e "$path" ]; then
    printf '| `%s` | ABSENT | already gone — deleting again is a no-op, not an error |\n' "$path"
    return
  fi
...
trigger_state() {
  ...
  elif grep -Eq '^[[:space:]]*#[[:space:]]*push:' "$path"; then
    printf '| `%s` | dispatch-only (push/PR still commented) |\n' "$path"
  elif grep -Eq '^[[:space:]]*push:' "$path"; then
    printf '| `%s` | ACTIVE on push/PR |\n' "$path"
  else
...
if [ ! -e "$DEPENDABOT" ]; then
  ...
  for eco in github-actions sbt npm pip gomod cargo docker; do
    if grep -Eq "^[[:space:]]*-[[:space:]]*package-ecosystem:[[:space:]]*\"?${eco}\"?" "$DEPENDABOT"; then
      printf '| `%s` | ACTIVE |\n' "$eco"
    elif grep -Eq "^[[:space:]]*#[[:space:]]*-[[:space:]]*package-ecosystem:[[:space:]]*\"?${eco}\"?" "$DEPENDABOT"; then
      printf '| `%s` | commented |\n' "$eco"
    else
...
PH="$(grep -rlE '\{\{[A-Za-z0-9_ -]+\}\}' --include='*.md' . 2>/dev/null | sed 's|^\./||' | sort || true)"
if [ -z "$PH" ]; then
  printf 'None.\n'
else
